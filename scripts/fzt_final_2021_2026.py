...
# -*- coding: utf-8 -*-
"""
最终版2021-2026年FZT公式回测（向量化优化 + 通达信SMA）
...
# ===================== 加载.bin格式数据 =====================
def load_all_stock_data_bin(data_dir):
    """一次性加载所有股票数据（.bin格式）"""
    try:
        ...
        print(f"   开始日期: {calendar_dates[0]}")
        print(f"   结束日期: {calendar_dates[-1]}")
...
def final_2021_2026_backtest():
    ...
    try:
        ...
        print("📊 使用所有股票数据（约5484只）")
        print("📅 时间范围: 2021-08-02 到 2026-02-06")
        print("=" * 80)
        ...
        df_fzt = calc_brick_pattern_final(
            df_raw=df,
            target_start_date='2021-08-02',
            target_end_date='2026-02-06'
        )
        ...
        print("⏰ 性能统计:")
        print("   -" * 20)
        print(f"   1. 数据加载: {data_load_time:.2f} 秒")
        print(f"   2. FZT指标计算: {calc_time:.2f} 秒")
        print(f"   3. 成功率计算: {eval_time:.2f} 秒")
        print(f"   {'-' * 40}")
        print(f"   📊 总耗时: {total_time:.2f} 秒")
        
        print("\n" + "=" * 80)
        print("✅ 最终版2021-2026年回测完成!")
        print(f"   处理股票数: {len(stock_codes)}")
        print(f"   总信号数: {total}")
        print(f"   总体成功率: {rate:.2f}%")
        ...
        print("\n🎯 优化效果评估:")
        print("   -" * 20)
        print(f"   📈 处理效率: {len(stock_codes) / total_time:.1f} 只股票/秒")
        print(f"   ⚡ 信号处理: {total / total_time:.1f} 个信号/秒")
        print(f"   🎯 优化目标达成: 从数小时缩短到 {total_time:.2f} 秒")
        
        # 7. 保存详细结果
        print("\n💾 保存详细结果...")
        results_dir = Path("results/fzt_final_2021_2026")
        ...
        with open(results_dir / "full_report.txt", 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("最终版2021-2026年FZT公式回测报告\n")
            f.write("=" * 80 + "\n\n")
            
            f.write("📋 回测配置\n")
            f.write("-" * 40 + "\n")
            f.write(f"   股票总数: {len(stock_codes)}\n")
            f.write(f"   时间范围: 2021-08-02 到 2026-02-06\n")
            ...
            f.write(f"   处理效率: {len(stock_codes) / total_time:.1f} 只股票/秒\n")
            f.write(f"   信号处理: {total / total_time:.1f} 个信号/秒\n\n")
            
            f.write("💡 关键发现\n")
            f.write("-" * 40 + "\n")
            f.write("   1. 原始FZT公式近期成功率约47%\n")
...
if __name__ == "__main__":
    sys.exit(final_2021_2026_backtest())
