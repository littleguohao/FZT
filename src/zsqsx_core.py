...
选股条件：当日的QSX > DKS（优化版，去掉close > DKS的条件）
...
def get_zsdkx_signal_conditions(
    ...
    选股条件：当日的QSX > DKS（去掉close > DKS的条件）
    ...
    # 不再需要close_gt_DKS列
    if 'close_gt_DKS' in df_signal.columns:
        df_signal = df_signal.drop(columns=['close_gt_DKS'])
